DROP TABLE IF EXISTS Collectibles_TEST;

-- Schema for creating a new table with collectibles content
CREATE TABLE Collectibles_TEST(
  ...
  is_visited BOOL SET DEFAULT false, -- check if the entity have already been visited  
  --year_of_visit YEAR, -- the year of visit, is used if we do not know exact date of visit, only remember the year
  --date_of_visit DATE, -- the exact date of visit
  date_of_visit TEXT,
  CONSTRAINT PK_Collectible PRIMARY KEY (qNumber,collection),   -- set the pair Q number and id of collection as unique primary key, the primary key value can be stored only once,
                                                                -- but for the entity it should be posibble to assing it to more collections 
  FOREIGN KEY (collection) REFERENCES Collections(collectionID) ON DELETE CASCADE   -- use collection id as foreign key, if the collection was deleted, 
