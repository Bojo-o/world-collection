...
class CollectibleDetailsQuery(QueryBuilder):
    ...
    def __init__(self, collectible_Qnumber: str):
        ...
        self.add_variable_into_select(
            "GROUP_CONCAT( DISTINCT ?valueLabel ;separator=\"<space>\")", "?values")
    ...
    def build_where_body(self):
        ...
        self.define_values_variable(
            "?supportedDataTypes", self._supported_data_type, "wikibase:")
        self.add_triple("?wd", "wikibase:propertyType", "?supportedDataTypes")
        self.filter_wrapper("?wd not in ({})".format(
            self.convert_set(self._property_exception, ", wd:")[1:]))

        self.add_triple("?wd", "wikibase:propertyType", "?dataType")
        self.add_triple("?statement", "?ps", "?value")

        self.optional_wrapper(
            "?statement ?psv [wikibase:timePrecision  ?timePrecision].")
        self.optional_wrapper(
            "?statement ?psv [wikibase:quantityUnit  ?unitValue].")

        self.add_label_servise(
            "?wd rdfs:label ?property . ?value rdfs:label ?valueLabel. ?unitValue rdfs:label ?unit.")
