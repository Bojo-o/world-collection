...
class AdministrativeAreaCollectiblesSearchQueryBuilder(CollectiblesSearchQueryBuilder):
    ...
    def build_class_and_location_restriction(self):
        if self._administrative_area is None:
            raise AdministrativeAreaCollectiblesSearchQueryBuilderException(
                "The area of search was not set")

        self.build_class_restriction()

        self.add_triple("?item", "wdt:{}".format(
            self._IN_ADMIN_AREA), "wd:{}".format(self._administrative_area))
        self.add_gearing_forward_hint()

        if self._administrative_area_exceptions.__len__ != 0:
            self.restriction_wrapper("?item", "?areaExceptions", "wdt:{}".format(
                self._IN_ADMIN_AREA), self._administrative_area_exceptions, True, True)
