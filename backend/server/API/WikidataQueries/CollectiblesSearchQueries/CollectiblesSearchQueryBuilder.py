...
class CollectiblesSearchQueryBuilder(QueryBuilder, ABC):
    ...
    def __init__(self, parent_class: str):
        ...
        self.add_variable_into_select(
            "GROUP_CONCAT( DISTINCT ?instancesLabel;separator=\"/\")", "?subTypeOf")
    ...
    def build_class_restriction(self):
        ...
        if self._parent_class != self._ANYTHING:
            self.add_triple("?item", self._INSTANCE_OR_SUBCLASS,
                            "wd:{}".format(self._parent_class))
    ...
    def add_quantity_filter(self, property: str, comparison_operator: ComparisonOperator, quantity_value: int, value_in_unit_QNumber: str = None):
        ...
        '''
        filter = QuantityFilterData(
            property, comparison_operator, quantity_value, value_in_unit_QNumber)
    ...
    def build_where_body(self):
        ...
        for itemFilter in self._item_filters:
            ...
            self.add_triple("?item", "wdt:{}".format(
                item.property), "wd:{}".format(item.value))
        ...
        for timeFilter in self._time_filters:
            ...
            self.filter_wrapper("\"{}\"^^xsd:dateTime {} {} ".format(
                time.date_value, time.comparison_operator.value, temp_name))
            i = i + 1
        ...
        for quantityFilter in self._quantity_filters:
            ...
            if unit is None:
                self.add_triple("?item", "wdt:{}".format(
                    quantity.property), temp_name)
                self.filter_wrapper("{} {} {}".format(
                    str(quantity.quantity_value), quantity.comparison_operator.value, temp_name))
            else:
                self.add_triple("wd:{}".format(unit),
                                "wdt:P2370", temp_name + "conversion")
                self.add_triple("?item", "p:{}/psn:{}/wikibase:quantityAmount ".format(
                    quantity.property, quantity.property), temp_name)
                self.filter_wrapper("({} * {}) {} {}".format(str(quantity.quantity_value),
                                    temp_name + "conversion", quantity.comparison_operator.value, temp_name))
            j = j + 1
