...
class RegionCollectiblesSearchQueryBuilder(CollectiblesSearchQueryBuilder):
    ...
    def build_class_and_location_restriction(self):
        if self._region is None:
            raise RegionCollectiblesSearchQueryBuilderException(
                "The region of search was not set")

        self.build_class_restriction()

        self.add_triple(
            "?countries", "wdt:{}*".format(self._PART_OF), "wd:{}".format(self._region))
        self.add_triple("?countries", "wdt:{}".format("P31"),
                        "wd:{}".format(self._COUNTRY_CLASS))
